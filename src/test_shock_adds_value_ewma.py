"""EWMA-basis re-run of the formal OOS R^2 test on shock-type predictive value.

Identical to code/test_shock_adds_value.py except that the per-day shock flags
are taken from data/aux/ewma_shocks_panel.parquet (rolling EWMA-126 cutoff)
instead of the full-sample shock columns in panel_with_shocks.parquet.

This isolates whether the previously-observed shock-type DR^2 (~+2.94pp full
panel; full interaction M2 -> M3) reflects a real structural signal or a
look-ahead-cutoff artifact.

Substitution: the columns oil_shock, dollar_shock, rate_shock, banking_shock,
gprd_threat_shock are over-written with their *_ewma counterparts (joined on
date index). All other code paths (M0..M4 design, 5-fold CV, permutation,
verdict logic) are identical to the original script.

Outputs:
  - output/stage3a/results/shock_adds_value_test_ewma.json
  - output/stage3a/shock_adds_value_test_ewma.md
  - output/stage3a/tables/shock_adds_value_test_ewma.tex
"""
...
EWMA_PATH = ROOT / "data/aux/ewma_shocks_panel.parquet"
...
OUT_JSON = RESULTS_DIR / "shock_adds_value_test_ewma.json"
OUT_MD = OUT_DIR / "shock_adds_value_test_ewma.md"
OUT_TEX = TABLES_DIR / "shock_adds_value_test_ewma.tex"
...
EWMA_COL_MAP = {s: f"{s}_ewma" for s in RETAINED_SHOCKS}
