...
EWMA_PATH = ROOT / "data/aux/era_conditional_walkforward_shocks_panel.parquet"
...
OUT_JSON = RESULTS_DIR / "shock_adds_value_test_walkforward.json"
OUT_MD = OUT_DIR / "shock_adds_value_test_walkforward.md"
OUT_TEX = TABLES_DIR / "shock_adds_value_test_walkforward.tex"
...
EWMA_COL_MAP = {s: f"{s}_wf" for s in RETAINED_SHOCKS}
...
def build_panel():
    ...
    # CRITICAL: do NOT fillna(0). NaN means "we couldn't classify this day
    # under the walk-forward cutoff" (operator literally had no within-era
    # cutoff yet during warmup). Treating those as "no shock" would be a
    # false assertion. Drop rows where any shock indicator is NaN.
    ...
    for orig, ewma_name in EWMA_COL_MAP.items():
        panel[orig] = panel[ewma_name]  # keep as float/NaN for now
    panel = panel.drop(columns=ewma_cols)
    # Drop warmup rows where any shock is NaN
    n_before_drop = len(panel)
    panel = panel.dropna(subset=RETAINED_SHOCKS)
    for s in RETAINED_SHOCKS:
        panel[s] = panel[s].astype(int)
    print(f"[walkforward] dropped {n_before_drop - len(panel)} warmup rows "
          f"({(n_before_drop - len(panel))/n_before_drop*100:.1f}% of panel)",
          flush=True)
    # --------------------------------
